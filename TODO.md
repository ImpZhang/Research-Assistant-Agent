...
- Extend the bounded Advisor read-tool plan with replay workflow generation and richer live replay executors. Current baseline: opt-in model-ranked read-tool selection is candidate-validated with deterministic fallback, failed Advisor read tools are captured as failed `ToolCallRecord` rows plus `advisor_tool_failure` replay cases, replay invocations can opt into `agent_replay` trace recording, and `context_search_miss` cases can opt into local live retrieval replay without external model calls.
