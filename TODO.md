...
- Extend the bounded Advisor read-tool plan with replay workflow generation and richer live replay executors. Current baseline: opt-in model-ranked read-tool selection is candidate-validated with deterministic fallback, failed Advisor read tools are captured as failed `ToolCallRecord` rows plus `advisor_tool_failure` replay cases, replay invocations can opt into `agent_replay` trace recording, and context-search/citation replay cases can opt into local live replay without external model calls.
...
- Extend the initial bad-case replay script with live replay executors for readiness false positives once those policies are bounded. Current baseline: context-search miss replay can execute local retrieval and validate required chunk/evidence/gap/idea ids plus minimum result counts; citation audit replay validates cited evidence existence, paper ownership, and required terms; `--record-run` persists replay summary and live executor tool calls for audit.
