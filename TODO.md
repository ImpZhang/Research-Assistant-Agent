...
- Current workflow durability baseline: `WorkflowStageRun` and `WorkflowArtifact` persist stage checkpoints, artifact hashes, config hashes, code commits, and failure taxonomy; `/research/jobs/{job_id}/lineage` exposes local inspection.
...
- Current real-paper evaluation baseline: 12 geolocalization/place-recognition papers completed strictly with `multimodal-embedding-v1`, async workflow polling, retrieval comparison, and benchmark profile execution; new local deployments now default the embedding template to `text-embedding-v1` for the text-first PDF retrieval path. A one-paper strict `text-embedding-v1` smoke now passes on GeoRanker with 47 external embeddings and benchmark `1 / 1`; full strict qwen3-rerank evaluation is currently blocked by provider quota (`AllocationQuota.FreeTierOnly`) rather than local code. Current query-evidence baseline: `scripts/build_geoloc_eval_dataset.py` and `scripts/check_geoloc_eval_dataset.py` generate and validate an ignored local 75-item query-evidence set plus 30 replay cases. Current hard-question regression baseline: `configs/geoloc_hard_questions.v1.jsonl`, `scripts/build_geoloc_hard_questions.py`, and `scripts/check_geoloc_hard_questions.py` generate and validate 20 researcher-style hard questions across all 12 papers with strict retrieval/replay checks. Current realistic metric baseline: `configs/geoloc_realistic_gold.v1.jsonl`, `scripts/build_geoloc_realistic_eval.py`, and `scripts/check_geoloc_realistic_eval.py` run 20 reviewer-labeled blind questions over the 12-paper corpus without per-query paper filters, currently reporting primary hit@8 `0.65`, MRR `0.3780`, and 7 primary-miss failure replay cases plus miss taxonomy. Next evaluation credibility work is manual review of sampled mappings and larger benchmark/prediction evidence, not merely adding more PDFs.
...
- Improve page/figure/table-aware PDF evidence extraction. Current P3 baseline handles noisy front matter, more heading variants, and chunk-level evidence top-up for long sparse text.
