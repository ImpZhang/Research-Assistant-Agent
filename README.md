...
- Production-readiness checks with `/health/ready`, SQLite storage readiness, API-key auth readiness, Workbench asset readiness, model-provider configuration visibility, request-id response headers, external-literature configuration checks, default-project scope contract, health build metadata, optional API-key protection for `/research/*`, Dockerfile, and docker-compose pilot deployment.
