...
- Browser workbench for API-key-backed pilot access, first-run empty/API-key/network states, pilot launch status, profile editing, upload, workflow launch, job tracking/cancel/retry, search, advisor chat/action sessions, cockpit, readiness, quality gates, decision, audit, bundle export, and dossier preview.
