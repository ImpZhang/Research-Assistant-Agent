...
- benchmark evidence readiness 的推荐动作需要能生成 ResearchTask，进入任务板执行，避免证据缺口只停留在报告中。
...
project bundle release 需要支持客户/导师 feedback 与验收记录：系统应在具体 release 下保存 feedback status、signoff confirmed、feedback notes、requested changes、blockers 和 accepted artifacts，并生成 Markdown feedback report。feedback 需要能一键转成 task board 任务，使用 `owner_type=project_bundle_release_feedback`、`due_phase=project_bundle_release_feedback_follow_up`，并写入 release->feedback 与 feedback->task 图边。project handoff bundle 必须包含最新 feedback JSON/Markdown artifacts，让外部接手者能看到对方是否接受交付、仍要求改什么、签收是否完成。

project bundle release 需要提供 closeout report：系统应把 release follow-up progress、最新 feedback、feedback task progress、blockers、signoff state 和 next actions 聚合成 closeout status（例如 awaiting_feedback、blocked、changes_requested、follow_up_open、awaiting_signoff、closed）。只有签收确认且 release/feedback follow-up tasks 都清完时才能标记 ready_to_close。project handoff bundle 必须包含最新 closeout JSON/Markdown artifacts，让客户/导师交付具备明确收口判定。

project bundle release closeout 需要能一键转成 task board 任务：系统应从 closeout blockers、next actions、signoff gaps 和 ready-to-close archive checks 生成后续任务，使用 `owner_type=project_bundle_release_closeout`、`due_phase=project_bundle_release_closeout_follow_up`，并写入 release->closeout 与 closeout->task 图边。这样交付收口报告不是只读判断，而是可以继续驱动验收、阻塞解除、签收确认和归档动作。
...
project bundle release acceptance snapshot 需要支持 compare/export/tasks：系统应允许对同一 release 的 baseline/candidate acceptance snapshots 进行比较，输出 acceptance_status/signoff/closeout delta、remaining_actions 增删、checklist open/resolved 变化和 Markdown change report。comparison 必须能进入 project handoff bundle metadata/latest artifact/manifest；当出现新增 remaining actions、checklist regressions 或 status/signoff regression 时，系统应能生成 `project_bundle_release_acceptance_packet_snapshot_comparison` owner_type 的任务，并写入 `project_bundle_release_acceptance_comparison_creates_task` graph edge。

project bundle release 需要提供 review session：系统应把 release note、progress、feedback、closeout、acceptance packet、latest acceptance snapshot 和 acceptance snapshot comparison 汇总成一次客户/导师评审会话，输出 review_status、ready_for_review、agenda、decisions_needed、risk_items、follow_up_actions、artifact_links 和 Markdown review brief。review session 必须能生成 `project_bundle_release_review_session` owner_type 的任务，写入 `project_bundle_release_review_creates_task` graph edge，并进入 project handoff bundle 的 metadata/latest Markdown artifact/manifest 字段。

project bundle release review session 之后需要保存 review outcome：系统应在具体 release 下持久化 `project_bundle_release_review_outcome` brief，记录 participants、review_decision、outcome_notes、decisions、accepted_artifacts、follow_up_actions、risks 和 signoff_confirmed。outcome 必须支持 create/list/detail/markdown export，写入 `project_bundle_release_has_review_outcome` graph edge，并能生成 `owner_type=project_bundle_release_review_outcome`、`due_phase=project_bundle_release_review_outcome_follow_up` 的任务，写入 `project_bundle_release_review_outcome_creates_task` edge。project handoff bundle 必须包含 outcome metadata、per-outcome Markdown、latest Markdown artifact 和 manifest latest-decision/signoff/follow-up/risk 字段，让客户评审会后的结论、风险和后续动作可审计、可签收、可继续执行。

project bundle release review outcome 需要提供 progress tracking：系统应基于 `owner_type=project_bundle_release_review_outcome` 和 outcome id 聚合后续任务，返回 task count、open/done/blocked/todo/doing 计数、completion ratio、blocked tasks、next tasks 和 Markdown progress report。project handoff bundle 必须包含 latest review outcome progress JSON/Markdown artifacts，并在 manifest 中暴露 latest progress availability、completion ratio、open task count 和 blocked task count，让客户评审会后的执行状态可以被用户、导师或外部 agent 直接判断。

project bundle release review outcome 需要支持 signoff evidence records：系统应允许在具体 review outcome 下持久化 `project_bundle_release_review_outcome_signoff` brief，记录 signoff_decision、approver、signoff_notes、accepted_artifacts、conditions、evidence_links，以及创建时的 outcome progress snapshot。signoff 必须支持 create/list/detail/Markdown export，写入 `project_bundle_release_review_outcome_has_signoff` graph edge，并进入 project handoff bundle 的 metadata、per-signoff Markdown、latest signoff Markdown artifact 和 manifest latest-signoff 字段，让评审结论、签收条件、证据链接和未完任务状态可以被客户、导师或外部 agent 审计。
...
project bundle readiness 需要支持 snapshot comparison：用户可以选择 baseline/candidate readiness snapshot，对比 readiness score、level、missing required checks、recommended actions、quick actions 和 manifest signals 的变化，并导出 Markdown comparison。project handoff bundle 需要自动包含最新两份 readiness snapshot 的 comparison JSON/Markdown，使客户或导师能看到每轮交付准备度的进步、退步和仍未解决项。

project bundle readiness snapshot comparison 需要能一键转成 task board 任务：新增 missing required checks、新增 recommended actions 和新增 quick actions 需要生成 `owner_type=project_bundle_readiness_snapshot_comparison`、`due_phase=bundle_readiness_change_follow_up` 的任务，并写入 `project_bundle_readiness_comparison_creates_task` 图边。若两轮对比没有新增事项，也要生成人工复核任务，确保交付准备度变化报告有人签收。
...
Workbench 的后段 delivery workflow 也需要复用同一套空状态：缺少 project bundle release note、release feedback、acceptance snapshot、review outcome、signoff evidence、bundle readiness snapshot、triage snapshot 或 research plan 时，前端应提示需要先完成哪个上游动作，而不是把这些情况混作普通 API 错误。
...
- brief 需要包含相关 research execution plans、plan task progress、readiness signals、evidence ledger signals、claim validation queue/task/result signals、triage signals 和 latest triage snapshot comparison。
...
系统需要提供 project onboarding readiness，作为客户第一次进入系统时的准备度检查：聚合 research profile、paper/evidence ingestion、first workflow、task board、project bundle、advisor loop、API key guard 和 MCP bridge setup，输出 readiness score、required checks、missing required checks、recommended actions、quick actions 和 Markdown report。它回答“现在能不能交给客户试用、还差哪几步”，而不是替代后续 project cockpit。

系统需要提供 project setup wizard，作为 onboarding readiness 的写入侧入口：客户可以一次性填写项目名、研究领域、active questions、target venues、method preferences、resource constraints、timeline、success criteria 和 first milestone。系统必须将这些内容保存到同一份 research profile，并立即返回更新后的 onboarding readiness、recommended next steps、quick actions 和 Markdown setup report，避免用户在 profile、onboarding 和 cockpit 之间手动拼状态。

project onboarding 需要能一键转成 task board 任务：从 required missing checks 和 optional pilot guardrails 创建 `owner_type=project_onboarding`、`due_phase=onboarding_follow_up` 的任务，并写入 `project_onboarding_creates_task` 图边。这样首次试点不是只得到准备度报告，而是能直接形成上传论文、运行 workflow、补安全配置、检查 MCP bridge 等可追踪行动项。
...
pilot status report 需要支持持久化 snapshot：用户可以保存当前报告为 `scope=pilot_report` 的 brief，并支持 list、detail 和 Markdown export。这样客户周报、导师会和试点交付不依赖实时状态漂移，可以保留当时的判断、风险、next actions 和 key metrics。
...
系统需要提供 project cockpit dashboard，作为客户进入项目后的第一屏能力：聚合 setup status、workflow stages、关键 metrics、readiness、quality gate、opportunity radar、claim validation result、risk alerts、highlights、primary next action 和 quick actions。它必须能回答“现在项目处于哪一阶段、成熟度怎样、最大风险是什么、下一步该点哪里”，避免用户在多个内部报告之间来回拼状态。
