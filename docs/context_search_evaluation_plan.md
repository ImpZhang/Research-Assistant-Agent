...
Prefer pytest fixtures for small deterministic checks. Add a script only if metric output becomes too large for unit tests. Committed fixtures now cover evidence hit@k, MRR, graph edge hit/noise checks, score breakdown coverage, exact phrase bonus scoring, score breakdown total consistency, paper-filter leak checks, no-match scoped queries, lexical-miss/vector-hit rescue, and `empty_query_guard_rate` for empty, too-short, and punctuation-only queries. Run `bash scripts/check_context_search_evaluations.sh` as the focused remote check before changing scoring or graph-expansion behavior.
