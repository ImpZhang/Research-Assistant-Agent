...
- Workbench availability is verified, a static demo-path contract protects the pilot flow from paper ingest through delivery closeout, browser inspection fixed refreshed-session restoration, the first viewport exposes a Latest Workflow continuation band, and Dossier now has a primary action bar with the full control surface behind Advanced Actions. Further human UX polish should focus on workflow grouping, copy, and real-pilot task sequencing.
