...
`/tasks/{task_id}/claim-validation-result` 为 claim validation follow-up task 写入 `claim_validation_result` task event。请求包含 validation_status、evidence_ids、notes、next_action 和 mark_task_done；默认会在记录结果后把 task 标记为 done。该接口复用 task event/history，不新增 artifact table，适合作为后续 MCP/agent 执行验证任务后的轻量结果回写入口。
...
`/ideas/{idea_id}/readiness` 将 evidence、novelty、proposal review、experiment analysis、decision memo、assumption audit、evidence ledger 和 task health 转成解释型 readiness score。响应包含总分、决策标签、score breakdown、blockers 和 Markdown report，用于判断是否可以继续深入执行，或者需要 targeted work、park、reject。
...
`/ideas/{idea_id}/quality-gate` 是 idea 级 go/no-go 质量门控。它读取 latest novelty check/refresh、readiness、proposal review、experiment analysis、decision memo、assumption audit、evidence ledger 和 task health，输出 gate score、required evidence、blocking risks、recommended actions 和 Markdown report。它与 readiness 的区别是：readiness 解释“准备度”，quality gate 决策“是否继续投入资源，或先 de-risk/revise/park/reject”。
