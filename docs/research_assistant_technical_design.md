...
生产化薄层由 `backend/app.py` 负责，而不混入科研 workflow service：`/health/ready` 检查 database、paper upload directory，并在 `WRITE_AUDIT_ENABLED=true` 时检查 write-audit directory；`API_KEY_AUTH_ENABLED=true` 时对 `/research/*` 启用 API key guard，支持 `X-Research-Assistant-Key` 与 `Authorization: Bearer`。`Dockerfile` 和 `docker-compose.yml` 提供单容器客户试点入口，compose 使用 `/app/data` volume 保存 SQLite 与上传文件，并默认要求设置 `API_KEY`。
...
Workbench dossier controls are grouped into unframed action groups for idea loop, task board, project delivery, and project operations. The grouping preserves existing element ids and JavaScript bindings while making the delivery workflow easier to scan.
