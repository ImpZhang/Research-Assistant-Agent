...
| 任务队列 | FastAPI BackgroundTasks + jobs 表 | MVP | 简单可靠；后续按 `docs/workflow_queue_design.md` 迁移到 durable worker queue。 |
...
`claims_json` stores claim_id, claim text, claim_type, support_level, supporting_evidence_ids, challenge_signals, and next_validation. `summary_json` stores claim_count, supported/unsupported counts, missing_evidence_count, high_risk_count, coverage_score, and decision_hint.
...
`/agent/metrics` 在这些 trace 表之上提供本地 observability 汇总：run_count、run_status_counts、run_type_counts、average_run_latency_ms、tool_call_count、tool_status_counts、tool_name_counts、tool_success_rate、replay_case_count、replay_verdict_counts、replay_case_type_counts、replay_run_status_counts、replay_live_executor_counts、replay_failed_run_count、replay_pass_rate 和 recent_failures。`/agent/metrics/export/markdown` 返回同一份指标的 Markdown 报告，方便本地 operator、面试演示、排障记录和后续 replay/guardrail 报告使用。这是轻量指标层，不替代 LangSmith、Langfuse 或 OpenTelemetry 等外部观测系统。
...
LangGraph 官方文档强调它适合长时间、有状态、可持久化、可流式输出、可人工介入的 agent/workflow。科研助手正好是多阶段状态机，因此保留 LangGraph 作为后续显式 workflow graph 的主线选择；当前已经稳定的 service-layer workflow 不因为依赖存在而重写。
...
`/benchmark-profiles` 暴露 benchmark profile registry：服务合并内置 profile 与项目本地 `configs/benchmark_profiles.json`，返回 runner 是否启用、command args 模板、required project paths、missing paths 和 runnable 状态。`/experiment-plans/{plan_id}/benchmark-run/execute` 支持 `profile_id`，当请求未显式传 `command_args` 时用 profile 填入 benchmark name、dataset、split、metric、working directory、timeout 和 config metadata。内置 `geoloc-country-accuracy-jsonl` profile 调用 `scripts/benchmark_geoloc_predictions.py`，从 `data/benchmarks/geoloc/validation.jsonl` 与 `outputs/predictions/geoloc/validation.jsonl` 读取本地标注/预测，输出 `country_accuracy` 以及可选 mean/median geodesic km；真实数据和预测文件保持 untracked。

`/experiment-runs/compare` 将两次同 idea 的 benchmark `ExperimentRun` 比较为 `ResearchBrief(scope="benchmark_run_comparison")`。服务从 metric_results 中提取 primary metric 和所有可比较指标，计算 baseline value、candidate value、delta、direction 和 improved flag，返回 `improved/regressed/no_change/incomplete` 状态，并渲染 Markdown comparison brief。Workbench 的 `Compare Runs` 使用当前 experiment plan 最新两条 benchmark run 自动创建比较，帮助真实 benchmark 多轮执行形成可追溯证据链。

`/ideas/{idea_id}/benchmark-evidence/readiness` 是 benchmark 到 SOTA signoff 之间的只读证据门。它读取同一 idea 下的 benchmark runs 和 `benchmark_run_comparison` briefs，返回 completed run count、comparison count、latest comparison status、missing items、warnings、recommended actions 和 Markdown readiness report。它不替代人工 SOTA 判断，而是确保进入 signoff 前至少有完成的 benchmark 和比较证据。

`/ideas/{idea_id}/benchmark-evidence/readiness/tasks` 将 readiness 的 recommended actions 转成 `ResearchTask`，owner type 为 `benchmark_evidence_readiness`，due phase 为 `benchmark_evidence_follow_up`。服务按 action hash 做 open-task 去重，避免 Workbench 重复点击产生重复任务。Workbench 的 `Benchmark Tasks` 按钮调用该 endpoint，把缺 benchmark run、缺 comparison brief、缺 artifact 或 regression follow-up 直接推进任务板。

SOTA review package 和 signoff record 会嵌入 compact benchmark readiness summary。package Markdown 在人工 checklist 前展示 benchmark evidence 状态；signoff 的 `manual_gate_summary` 记录 benchmark readiness status、missing items 和 warnings，使 reviewer 同时看到 external search、nearest work、benchmark evidence 三类 gate。对于 `confirmed_novel` signoff，如果 benchmark evidence readiness 尚未满足 completed run + comparison brief，manual gate 会加入 `benchmark_evidence_not_ready` blocker，并让 `ready_for_sota_claim=false`；人工 signoff record 仍会保存，用于保留 reviewer 判断和后续补证据流程。
...
`/ideas/{idea_id}/evidence-ledger` 生成 `IdeaEvidenceLedger` artifact，将 idea/proposal 中的 core hypothesis、novelty argument、expected contribution 和 method/experiment summary 压成 claim-level ledger。服务会读取 idea evidence ids、latest proposal review、experiment analysis、novelty check、related-work matrix 和 assumption audit，生成 claims、evidence_links、counterevidence、missing_evidence、risk_register、coverage_score、decision_hint 和 Markdown report。创建后写入 `idea_has_evidence_ledger`、`evidence_ledger_tracks_claim`、`evidence_supports_claim` 图边，使“科研主张 -> 支持证据/反向信号/缺口”可被 GraphRAG-lite 与 MCP tool 追踪。

`/ideas/{idea_id}/evidence-ledgers/{ledger_id}/tasks` 将 evidence ledger 中的 unsupported/partially supported/challenged claims、missing evidence、counterevidence 和 risk register 转成 `ResearchTask` backlog，owner type 为 `idea_evidence_ledger`，due phase 为 `evidence_follow_up`，并写入 `evidence_ledger_creates_task` 图边。这样证据账本不是只输出报告，而能直接生成下一轮补证据、处理反证和降低风险的执行队列。

`/ideas/{idea_id}/evidence-ledgers/{ledger_id}/claims/{claim_id}/validation-packet` 读取单条 claim，聚合 supporting evidence、evidence links、claim challenge signals、ledger counterevidence、claim-specific missing evidence、ledger follow-up tasks、validation actions 和 graph edge summary，并输出 Markdown。它面向导师讨论、MCP tool 和后续 planner：调用方不需要解析完整 ledger，就能下钻到“这条科研主张到底由什么支撑、哪里缺证据、下一步做什么”。

`/claims/validation-queue` 扫描每个 idea 的 latest evidence ledger，将 claim 按 support level、supporting evidence count、missing evidence、counterevidence 和 linked task count 计算 urgency score，输出 priority、recommended action、summary 和 Markdown queue。它是项目级“证据补强看板”，用于每日 triage、advisor brief 前准备和后续 agent 自动挑选验证任务。

`/claims/validation-queue/tasks` 将 top claim validation queue items 转成 `ResearchTask`，使用 `owner_type=claim_validation_queue`、`due_phase=claim_validation_follow_up`，并把 ledger_id、claim_id、support_level、urgency_score 和 evidence gap counts 写入 task metadata。GraphRAG-lite 会写入 `idea_has_claim_validation_queue`、`claim_validation_queue_prioritizes_claim` 与 `claim_validation_queue_creates_task` 图边，让“薄弱 claim -> 验证任务 -> 后续 lineage/progress/packet”形成可追踪闭环。

`/tasks/{task_id}/claim-validation-result` 为 claim validation follow-up task 写入 `claim_validation_result` task event。请求包含 validation_status、evidence_ids、notes、next_action 和 mark_task_done；默认会在记录结果后把 task 标记为 done。该接口复用 task event/history，不新增 artifact table，适合作为后续 MCP/agent 执行验证任务后的轻量结果回写入口。idea progress、project overview 和 advisor brief 会聚合这些 result event，展示 status distribution、recent results 和下一步动作；readiness scoring 和 quality gate 会进一步把 supported/challenged/needs_more_evidence/inconclusive 转成 claim validation impact score、blocker 和 recommended action。
...
Proposal/workbench artifacts 会同步写入 GraphRAG-lite：`idea_has_proposal_draft`、`idea_has_novelty_check`、`novelty_check_creates_task`、`proposal_review_reviews_draft`、`proposal_revision_updates_draft`、`proposal_revision_addresses_review`、`proposal_revision_creates_task`、`idea_has_experiment_plan`、`experiment_plan_has_run`、`idea_has_experiment_run`、`task_records_experiment_run`、`experiment_run_has_analysis`、`idea_has_experiment_analysis`、`task_records_experiment_analysis`、`experiment_analysis_creates_task`、`idea_has_decision_memo`、`decision_memo_creates_task`、`idea_has_assumption_audit`、`idea_has_evidence_ledger`、`evidence_ledger_tracks_claim`、`evidence_supports_claim`、`evidence_ledger_creates_task`、`idea_has_claim_validation_queue`、`claim_validation_queue_prioritizes_claim`、`claim_validation_queue_creates_task`、`project_cockpit_creates_task`、`project_pilot_report_snapshot_creates_task`、`project_pilot_report_comparison_creates_task`、`project_advisor_chat_creates_task`、`idea_has_opportunity_radar`、`opportunity_radar_creates_task`、`task_board_snapshot_tracks_task`。这使得后续 context search、MCP tool 或 workflow planner 可以沿着 idea 的演化链路追踪到草案、novelty screen、评审、修订、执行任务、实验运行、实验结论、决策 memo、假设审计、证据账本、claim validation queue、cockpit 任务、pilot report snapshot 任务、pilot report comparison 任务、advisor chat 任务、机会雷达和下一轮 follow-up 任务。

`/ideas/{idea_id}/lineage` 将 idea 的 research plans、related-work matrices、proposal drafts、proposal reviews、proposal revisions、experiment runs、experiment analyses、decision memos、assumption audits、evidence ledgers、research tasks、task board snapshots 与 graph edge summary 聚合为一个响应，并提供 Markdown lineage export，方便前端和 MCP 一次性读取研究演化轨迹。

`/ideas/{idea_id}/progress` 将同一 idea 的 proposal、experiment、analysis、decision、assumption audit、evidence ledger、evidence follow-up task、research plan、blocker 和 snapshot 状态聚合为进度总览，返回 artifact counts、latest artifacts、task summary、experiment summary、blockers、recommended next step 和 Markdown report。task summary 会按 owner type 和 due phase 汇总，让 plan task 能和 proposal/analysis/decision/evidence follow-up task 一起排优先级。它面向 dashboard 和 MCP planner，不替代 lineage，而是回答“下一步该干什么”。

`/ideas/{idea_id}/research-packet` 面向导师讨论、MCP tool 和外部 planner，聚合 latest artifacts、latest evidence ledger、latest research plan、open tasks、graph edge summary 和 Markdown context。它不替代 lineage/progress，而是提供一个“拿来就能作为上下文”的单 idea packet，避免调用方每次自行拼接多个端点。

`/ideas/{idea_id}/timeline` 汇总 proposal、experiment、decision、assumption audit、evidence ledger、research plan 和 task event，按时间倒序输出 `IdeaTimelineEvent` 列表和 Markdown activity log。它面向导师汇报、handoff、复盘和后续 agent 接手，回答“这个 idea 最近发生过什么”。

`/ideas/{idea_id}/readiness` 将 evidence、novelty、proposal review、experiment analysis、decision memo、assumption audit、evidence ledger、claim validation result impact 和 task health 转成解释型 readiness score。响应包含总分、决策标签、score breakdown、blockers 和 Markdown report，用于判断是否可以继续深入执行，或者需要 targeted work、park、reject。
...
`/ideas/{idea_id}/quality-gate` 是 idea 级 go/no-go 质量门控。它读取 latest novelty check/refresh、readiness、proposal review、experiment analysis、decision memo、assumption audit、evidence ledger、claim validation result impact 和 task health，输出 gate score、required evidence、blocking risks、recommended actions 和 Markdown report。它与 readiness 的区别是：readiness 解释“准备度”，quality gate 决策“是否继续投入资源，或先 de-risk/revise/park/reject”。如果 claim validation challenged 关键主张，quality gate 不允许直接 advance；如果结果是 needs_more_evidence，则把 next_action 纳入 targeted revision。
...
`/advisor/chat` 是第一版自然语言项目顾问入口。请求包含 question、可选 idea_id/paper_ids、include_cockpit、include_context、context_limit 和 max_tool_calls；响应包含 agent_run_id、intent、answer、answer_markdown、cockpit_phase、readiness_level、recommended_actions、risk_alerts、tool_suggestions、cited_evidences、cited_gaps、cited_ideas 和 source_summaries。当前实现是 bounded deterministic advisor tool plan：先创建 `advisor_chat` AgentRun，再在白名单内选择 `get_project_cockpit`、`search_research_context`、`get_idea_progress`、`get_idea_lineage`、`list_research_tasks` 等 read tools，受 max_tool_calls 限制，并把 selected/skipped tools 和 observations 写入 `source_summaries.tool_plan`。每次 read call 都记录为 `ToolCallRecord`，最后生成 Markdown answer 和 tool suggestions。后续接入 LLM 时应复用这份 response contract 和 trace contract，把 tool plan/context 作为 prompt 输入，而不是把聊天逻辑和内部 route 拼接写死在前端。
...
`/export/project-bundle` produces an `application/zip` handoff package for the whole project. It includes project triage brief, persisted triage snapshots, latest triage snapshot comparison, pilot report snapshots, latest pilot report snapshot comparison, project progress overview, readiness overview, quality gate overview, opportunity radar, claim validation queue, recent task board state, persisted advisor briefs, research execution plans, plan progress reports, project bundle release notes, and JSON metadata. The bundle writes `06-claim-validation-queue.md` plus `metadata/claim-validation-queue.json`, `metadata/pilot-report-snapshots.json`, `metadata/pilot-report-snapshot-comparison.json`, `metadata/bundle-readiness-snapshots.json`, `metadata/bundle-readiness-snapshot-comparison.json`, `metadata/project-bundle-releases.json`, `artifacts/pilot/pilot-report-snapshot-*.md`, `artifacts/pilot/latest-pilot-report-snapshot-comparison.md`, `artifacts/readiness/project-bundle-readiness-snapshot-*.md`, `artifacts/readiness/latest-bundle-readiness-snapshot-comparison.md`, and `artifacts/releases/project-bundle-release-*.md`; it exposes queue counts, pilot snapshot counts, latest pilot comparison ids, readiness snapshot counts, latest readiness comparison ids/deltas, release note counts, latest release recipient/readiness, critical/high counts, priority buckets, and support-level buckets in `metadata/manifest.json`. It is the project-level counterpart to idea bundle export, meant for backups, advisor meetings, customer pilot reports, and downstream MCP/agent handoff.

`/export/project-bundle/releases` persists a project bundle release note as a `ResearchBrief` with `scope=project_bundle_release`. The summary captures recipient, free-form release notes, readiness score/level, latest readiness snapshot/comparison ids, and the current bundle manifest summary; the Markdown export renders a customer/advisor-facing handoff note with delivery signals and a handoff checklist. List/detail/Markdown endpoints expose saved releases, and `/export/project-bundle` includes release note JSON/Markdown artifacts plus latest release fields in the manifest.

`/export/project-bundle/releases/{release_id}/tasks` turns a saved release note into `ResearchTask` records with `owner_type=project_bundle_release` and `due_phase=project_bundle_release_follow_up`. Missing required checks become critical release blockers; handoff checks cover recipient receipt, claim queue review, open-task ownership, and release closeout. `ArtifactGraphService.link_project_bundle_release_tasks` writes `project_bundle_release_creates_task` edges so post-release work is traceable from the exact release note.
...
`/export/project-bundle/releases/{release_id}/closeout` is the read-only delivery closeout view for a release. It combines the release progress response, latest feedback record, all feedback-owned tasks, blocker lists, signoff state, and next actions into a `ProjectBundleReleaseCloseoutResponse`. Status is `awaiting_feedback` when no feedback exists, `blocked` when blockers or rejected feedback are present, `changes_requested`/`follow_up_open` while follow-up remains, `awaiting_signoff` when work is complete but signoff is missing, and `closed` only when release tasks, feedback tasks, and signoff are complete. `/export/project-bundle` writes `metadata/project-bundle-release-closeout.json`, `artifacts/releases/latest-project-bundle-release-closeout.md`, and closeout status/ready/action/blocker fields in the manifest.

`/export/project-bundle/releases/{release_id}/closeout/tasks` turns the computed closeout response into executable task-board work. The task service extracts blockers, next actions, signoff gaps, and ready-to-close archive checks into `ResearchTask` rows with `owner_type=project_bundle_release_closeout` and `due_phase=project_bundle_release_closeout_follow_up`; blockers are prioritized as critical and signoff gaps as high. `ArtifactGraphService.link_project_bundle_release_closeout_tasks` writes `project_bundle_release_has_closeout` and `project_bundle_release_closeout_creates_task` edges so the release, closeout decision, and generated follow-up work remain traceable.

`/export/project-bundle/releases/{release_id}/acceptance-packet` is the customer/advisor-facing release acceptance view. It recomputes the release progress, latest feedback, closeout response, and `project_bundle_release_closeout` tasks, then returns `ProjectBundleReleaseAcceptancePacketResponse` with `acceptance_status`, `ready_for_signoff`, `signoff_confirmed`, an acceptance checklist, remaining actions, open closeout tasks, and Markdown. `/export/project-bundle` writes `metadata/project-bundle-release-acceptance-packet.json`, `artifacts/releases/latest-project-bundle-release-acceptance-packet.md`, and manifest fields for acceptance status, signoff readiness, remaining action count, and open closeout task count.

`/export/project-bundle/releases/{release_id}/acceptance-packet/snapshots` persists the current acceptance packet as a `ResearchBrief` with scope `project_bundle_release_acceptance_packet`. The snapshot stores the source release id/title, acceptance status, signoff flags, closeout status, remaining actions, checklist, closeout-task summary, release-progress summary, latest feedback id, source generation time, persisted generation time, and the Markdown signoff packet. List/detail/export endpoints expose saved snapshots, `ArtifactGraphService.link_project_bundle_release_acceptance_packet` creates `project_bundle_release_has_acceptance_packet` edges, and `/export/project-bundle` writes `metadata/project-bundle-release-acceptance-packet-snapshots.json`, per-snapshot Markdown artifacts, the latest snapshot Markdown artifact, and manifest fields for snapshot count/latest id/release/status/signoff readiness.

`/export/project-bundle/releases/{release_id}/acceptance-packet/snapshots/compare` compares two persisted acceptance packet snapshots for the same release. It returns `ProjectBundleReleaseAcceptancePacketSnapshotComparisonResponse` with status/signoff/closeout deltas, remaining-action deltas, checklist open/resolved deltas, a summary, and Markdown. `/compare/export/markdown` exposes the Markdown report, `/compare/tasks` creates `ResearchTask` rows with `owner_type=project_bundle_release_acceptance_packet_snapshot_comparison` and `due_phase=project_bundle_release_acceptance_change_follow_up`, and `ArtifactGraphService.link_project_bundle_release_acceptance_snapshot_comparison_tasks` writes `project_bundle_release_acceptance_comparison_creates_task` graph edges. `/export/project-bundle` automatically compares the latest two acceptance snapshots for the latest release, then writes `metadata/project-bundle-release-acceptance-packet-snapshot-comparison.json`, `artifacts/releases/latest-project-bundle-release-acceptance-packet-snapshot-comparison.md`, and manifest fields for comparison availability, baseline/candidate ids, added-action count, and newly open checklist count.

`/export/project-bundle/releases/{release_id}/review-session` is the customer/advisor release review surface. It composes the release note, release progress, latest feedback, closeout, acceptance packet, latest acceptance snapshot, acceptance snapshot comparison, review-owned task summary, agenda, decisions, risks, follow-up actions, artifact links, and Markdown into `ProjectBundleReleaseReviewSessionResponse`. `/review-session/tasks` creates `ResearchTask` rows with `owner_type=project_bundle_release_review_session` and `due_phase=project_bundle_release_review_follow_up`; `ArtifactGraphService.link_project_bundle_release_review_tasks` writes `project_bundle_release_review_creates_task` graph edges. `/export/project-bundle` writes `metadata/project-bundle-release-review-session.json`, `artifacts/releases/latest-project-bundle-release-review-session.md`, and manifest fields for review availability, status, readiness, decision count, risk count, and follow-up count.

`/export/project-bundle/releases/{release_id}/review-session/outcomes` persists post-meeting review outcomes as `ResearchBrief` records with `scope=project_bundle_release_review_outcome`. The summary stores release id/title, recipient, review/acceptance status, review decision, participants, outcome notes, decisions, accepted artifacts, follow-up actions, risks, signoff state, source review generation time, and persisted generation time; the Markdown export renders the outcome plus source-session counters. List/detail/export endpoints expose saved outcomes. `ArtifactGraphService.link_project_bundle_release_review_outcome` writes `project_bundle_release_has_review_outcome`, while `/outcomes/{outcome_id}/tasks` creates `ResearchTask` rows with `owner_type=project_bundle_release_review_outcome`, `due_phase=project_bundle_release_review_outcome_follow_up`, and `project_bundle_release_review_outcome_creates_task` graph edges. `/export/project-bundle` writes `metadata/project-bundle-release-review-outcomes.json`, per-outcome Markdown artifacts, `artifacts/releases/latest-project-bundle-release-review-outcome.md`, and manifest fields for latest outcome id/release/decision/signoff/follow-up/risk counts.

`/export/project-bundle/releases/{release_id}/review-session/outcomes/{outcome_id}/progress` summarizes follow-up execution for one persisted review outcome. It filters `ResearchTask` rows by `owner_type=project_bundle_release_review_outcome` and outcome id, reuses the release task summary/order helpers, returns blocked/open/done/next tasks plus Markdown, and exposes completion ratio, open count, and blocker count. `/export/project-bundle` writes `metadata/project-bundle-release-review-outcome-progress.json`, `artifacts/releases/latest-project-bundle-release-review-outcome-progress.md`, and manifest fields for latest outcome progress availability, completion ratio, open task count, and blocked task count.

`/export/project-bundle/releases/{release_id}/review-session/outcomes/{outcome_id}/signoffs` persists signoff evidence as `ResearchBrief` records with `scope=project_bundle_release_review_outcome_signoff`. The summary stores release id, outcome id/title, recipient, review decision, outcome signoff state, signoff decision, approver, notes, accepted artifacts, conditions, evidence links, and the current outcome progress summary/ratio/open/blocker counters; Markdown renders the signoff, evidence, conditions, and next-task snapshot. List/detail/export endpoints expose saved signoffs, and `ArtifactGraphService.link_project_bundle_release_review_outcome_signoff` writes `project_bundle_release_review_outcome_has_signoff`. `/export/project-bundle` writes `metadata/project-bundle-release-review-outcome-signoffs.json`, per-signoff Markdown artifacts, `artifacts/releases/latest-project-bundle-release-review-outcome-signoff.md`, and manifest fields for latest signoff id/release/outcome/decision/confirmation/approver/progress.
...
`/export/project-bundle/readiness/snapshots` persists the current bundle readiness as a `ResearchBrief` with `scope=bundle_readiness`. The summary stores readiness score/level, required check counts, missing checks, recommended actions, quick actions, and the bundle manifest summary; the Markdown export stores the human-readable delivery audit. `/export/project-bundle` includes saved readiness snapshots in `metadata/bundle-readiness-snapshots.json` and `artifacts/readiness/project-bundle-readiness-snapshot-*.md`, and exposes latest readiness snapshot fields in `metadata/manifest.json`.
...
`/onboarding/tasks` converts onboarding readiness gaps into `ResearchTask` records with `owner_type=project_onboarding` and `due_phase=onboarding_follow_up`. Required missing checks become high/critical setup work, while optional guardrails such as API-key protection or MCP bridge review can be included for customer pilot hardening. `ArtifactGraphService.link_project_onboarding_tasks` writes `project_onboarding_creates_task` edges so onboarding work appears in graph traces, task boards, project bundles, and later agent handoff.
...
`/pilot/report/snapshots/compare/tasks` turns a saved pilot report snapshot comparison into `ResearchTask` rows with `owner_type=project_pilot_report_snapshot_comparison` and `due_phase=pilot_report_change_follow_up`. It prioritizes newly added risks as critical, newly added next actions as high, quick actions as medium, and falls back to a review task when no added item exists. `ArtifactGraphService` writes `project_pilot_report_comparison_creates_task` edges so weekly report changes remain traceable.
...
`scripts/mcp_http_bridge.py` is the first runnable MCP bridge. It is a dependency-light stdio JSON-RPC adapter that implements `initialize`, `tools/list`, and `tools/call`, loads `/research/tools/mcp-spec`, and forwards calls to the FastAPI HTTP routes. Binary bundle responses are returned as base64 text payloads so MCP clients can still receive zip exports without SDK-specific extensions. The bridge also supports `--read-only`, repeated `--allow-tool`, repeated `--deny-tool`, API-key forwarding, project-scope forwarding, request-id error correlation, environment-variable policy configuration, and `--health-check` JSON output for managed deployment checks.

Upload ingestion validates extension, size, and lightweight content signatures before writing files to `PAPER_UPLOAD_DIR`; operators can tune `PAPER_UPLOAD_ALLOWED_EXTENSIONS` and `PAPER_UPLOAD_MAX_BYTES` for pilot deployments. Text/Markdown uploads must be UTF-8 text without null bytes, and PDF uploads must start with a PDF header. User/project scoping is designed in `docs/user_project_scoping_design.md` and should not be implemented before migration tooling and auth identity are explicit.

生产化薄层由 `backend/app.py` 负责，而不混入科研 workflow service：`/health/ready` 检查 database、SQLite database storage parent、API-key auth configuration、request-id header configuration、Workbench static assets、model-provider configuration、paper upload directory，并在 `WRITE_AUDIT_ENABLED=true` 时检查 write-audit directory；`API_KEY_AUTH_ENABLED=true` 时对 `/research/*` 启用 API key guard，支持 `X-Research-Assistant-Key` 与 `Authorization: Bearer`；所有 HTTP 响应都会回显配置的 request-id header，写操作审计复用同一 request id。`Dockerfile` 和 `docker-compose.yml` 提供单容器客户试点入口，compose 使用 `/app/data` volume 保存 SQLite 与上传文件，并默认要求设置 `API_KEY`。
...
Workbench dossier controls are grouped into unframed action groups for idea loop, task board, project delivery, and project operations. The grouping preserves existing element ids and JavaScript bindings while making the delivery workflow easier to scan. The sidebar runtime readiness strip reads `/health/ready` and shows compact database, storage, auth, Workbench-asset, model-provider, and external-literature status badges. `/research/status` reports `runtime_readiness_signals` and `representative_paper_review_protocol` so clients can discover that the deployment health checks and human-review gate are present. Workbench also loads `/research/project/scope` to show active scope compatibility before users mistake a project id for isolation. Workbench learns the configured request-id response header from readiness, captures that header, and appends a short request id to connection and error states for support correlation.
...
Idea -> has_evidence_ledger -> IdeaEvidenceLedger
IdeaEvidenceLedger -> tracks_claim -> Claim
Evidence -> supports_claim -> Claim
IdeaEvidenceLedger -> creates_task -> ResearchTask
ProjectPilotReportSnapshot -> creates_task -> ResearchTask
ProjectPilotReportSnapshotComparison -> creates_task -> ResearchTask
```
...
- 写操作审计按 `docs/write_operation_audit_design.md` 的 JSONL-first 方案实现第一版原型：`WRITE_AUDIT_ENABLED=true` 时，非 GET `/research/*` middleware 会把 sanitized metadata 写到 `WRITE_AUDIT_DIR/write-operations.jsonl`，记录 method、route template、operation、entity type、status、request id、client label、API key fingerprint prefix、policy、duration 和 query key names，不读取 request body，不记录 payload/secrets。`AUDIT_ADMIN_EXPORT_ENABLED=true` 时才注册 `GET /research/admin/write-audit/summary`，并要求独立 admin key header；普通 pilot API key 不能单独读取审计 summary；数据库化需等迁移方案确定。

## 17. DeerFlow 决策

结论：不迁移 DeerFlow。P6 revisit 记录在 `docs/graphrag_langgraph_deerflow_evaluation.md`，当前只把 DeerFlow 视为未来可通过 tool manifest/MCP bridge 接入的外部 planner，而不是核心运行时。
...
迁移前置条件记录在 `docs/workflow_queue_design.md`。在 operator 确认部署拓扑、队列后端、worker 进程管理、数据迁移和备份策略前，不应直接引入 Redis/RQ/Celery/Dramatiq/Temporal 依赖或修改生产启动方式。

保留的 API 合同：

- `POST /research/workflows/literature-to-ideas/async` 继续立即返回 `JobRead`。
- `GET /research/jobs/{job_id}` 继续作为前端、MCP bridge 和 smoke workflow 的轮询入口。
- `GET /research/jobs/{job_id}/artifacts` 继续返回完整产物快照。
- cancel/retry 语义应在独立 worker 下保持兼容，并明确 pending、leased、running、failed、canceled 的状态转换。

推荐迁移路径仍然是渐进式：

```text
FastAPI BackgroundTasks
-> RQ/Redis or DB-backed worker lease
-> Celery/Dramatiq only if broader queue ecosystem is needed
-> Temporal only if resumable DAG state becomes central
```
...
### ADR-002：保留 LangGraph 作为显式 workflow graph 方向

决定：

- 保留 LangGraph 依赖和架构方向。
- 当前稳定的 service-layer workflow 不做整体重写。
- 新的复杂 workflow 只有在需要 resumable DAG、checkpoint、streaming 或 human-in-the-loop 时才用 LangGraph 实现。

原因：

- 当前依赖和设计已为 LangGraph 留出位置。
- 现有 workflow routes、jobs trace、artifact outputs 和 tests 已经稳定。
- 官方定位适合 long-running/stateful agent orchestration，但应先用于真正需要 DAG 语义的新流程。
