...
- 写操作审计先按 `docs/write_operation_audit_design.md` 的 JSONL-first 方案实现：非 GET `/research/*`、Workbench、script 和 MCP bridge 写入都应记录 sanitized metadata、request id、policy 和结果，不记录 payload/secrets；数据库化需等迁移方案确定。
