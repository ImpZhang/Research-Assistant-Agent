...
- Added focused upload guardrail coverage for allowed-extension overrides, allowed-extension normalization, case-insensitive extension matching, invalid/non-positive max-byte fallback, empty uploads, Markdown default-extension ingestion, path-traversal filename sanitization, and non-UTF-8 text rejection before file writes.
...
- Added `scripts/check_research_workflow_primitives.sh` as a focused remote check for local literature search, external provider config normalization/status aggregation, OpenAlex/arXiv/Semantic Scholar provider parsers, paper cards, structured extraction fallback, gap/idea generation, novelty screening, related-work matrices, and Markdown dossier exports.
...
- Run `bash scripts/check_research_workflow_primitives.sh` before changing local literature search, external provider config normalization/status aggregation, paper card extraction, structured extraction fallback, gap/idea generation, novelty screening, related-work matrices, or Markdown dossier exports.
...
- Added paper-filter leak evaluation fixtures so scoped context-search evidence, gaps, ideas, and GraphRAG-lite context cannot include excluded papers.
- Added a graph expansion recall fixture so relevant seed-node edges cannot be hidden by many newer unrelated graph edges.
- Added multi-edge-type filter coverage so GraphRAG-lite context search can include selected workflow edge families without admitting unrelated edge types.
- Normalized blank, duplicate, and whitespace-padded GraphRAG-lite edge-type filters so context search does not drop valid selected edge families.
...
- Added `scripts/check_graph_rag_lite.sh` as the focused remote check for duplicate-edge reuse, duplicate/orphan-edge stats, and graph link/stat fixtures.
