...
- Added `scripts/check_context_search_evaluations.sh` as the focused remote check for context-search ranking, no-match, phrase-bonus, vector-rescue, query-term deduplication, lower/upper limit-clamping, guard, filter normalization, score-breakdown, and graph-context fixtures.
