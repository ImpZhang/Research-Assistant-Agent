...
require_tokens(
    ...
    [
        ...
        "Cold backup is the preferred first-pilot path",
        "Restore should never write over a live service volume.",
        "Prefer restoring into a new empty target volume",
        "After restore or volume swap, verify `/health/ready`, authenticated `/research/status`, Workbench Pilot Launch, and a known project bundle or paper record",
