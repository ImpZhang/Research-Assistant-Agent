...
def test_research_status() -> None:
    ...
    assert "workbench_project_scope_forwarding" in body["implemented_capabilities"]
    ...
    assert "benchmark_evidence_task_generation" in body["implemented_capabilities"]
...
def test_tool_manifest_lists_mcp_ready_research_tools() -> None:
    ...
    assert "create_tasks_from_benchmark_evidence_readiness" in names
...
def test_workbench_static_assets_are_served() -> None:
    ...
    assert "benchmarkTasksButton" in response.text
    ...
    assert "refreshProjectScopeStatus" in script.text
    assert "projectScope" in script.text
    assert "/research/project/scope" in script.text
    ...
    assert "createBenchmarkEvidenceTasks" in script.text
    assert "/benchmark-evidence/readiness" in script.text
    assert "/benchmark-evidence/readiness/tasks" in script.text
    assert (
